// SSA 完整链（修复：edt/sid 单次生成，贯穿所有步骤）
...
function generateSimulate(mid, uid, dfid, webglHash) {
    ...
    const sid = crypto.publicEncrypt({ key: OAEP_PEM, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash: 'sha256' }, Buffer.from(key, 'utf8')).toString('base64');
    return { edt, sid };
}
...
async function main() {
    ...
    const body1 = JSON.stringify({ eventid: ssaCode, userid: Number(USERID), platid: 2, rtype: 1, wasm: 1, i: '', sid: sidEnc, edt: edtEnc });
    ...
    if (r3.json?.status === 1) {
        console.log('\n✅✅✅ SSA 验证通过！需要把 response 中的 token/cookie 信息带回 v5 重试');
    } else if (r3.json?.error_code === 36010) {
        console.log('\n⚠️  需要腾讯滑块验证码 (v_type=' + vType + ', error_code=36010)');
        console.log('   txappid: ' + r3.json?.txappid);
    } else {
        console.log('\n⚠️  error_code: ' + r3.json?.error_code + ' msg: ' + r3.json?.error_msg + ' data: ' + r3.json?.data);
    }
}
